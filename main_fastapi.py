...
@app.post("/buttons/", response_model=CRUD.Button)
...
@app.post("/categories/", response_model=CRUD.Category)
...
@app.get("/categories/", response_model=list[CRUD.Category])
def get_categories(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return methods.get_categories(db, skip=skip, limit=limit)


@app.get("/categories/{category_id}", response_model=CRUD.Category)
