...
def create_scope(scope_inp: ScopeCreate, user=Depends(UnionAuth(['services.scope.create']))):
...
def delete_scope(scope_id: int, user=Depends(UnionAuth(['services.scope.delete']))):
    scope = db.session.query(Scope).filter(Scope.id == scope_id).one_or_none()
    if not scope:
        raise HTTPException(status_code=404, detail="Scope does not exist")
...
def update_scope(scope_inp: ScopeUpdate, scope_id: int, user=Depends(UnionAuth(['services.scope.update']))):
    scope = db.session.query(Scope).filter(Scope.id == scope_id).one_or_none()
    if not scope:
        raise HTTPException(status_code=404, detail="Scope does not exist")
