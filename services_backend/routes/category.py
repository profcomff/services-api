...
def create_category(category_inp: CategoryCreate, user: dict[str, str] = Depends(UnionAuth(["services.category.create"]))):
...
def get_categories(offset: int = 0, limit: int = 100, user: dict[str, str] = Depends(UnionAuth(allow_none=True))):
...
def get_category(category_id: int, user: dict[str, str] = Depends(UnionAuth(allow_none=True))):
    if user:
        category = db.session.query(Category) \
                .filter(or_(bool(db.session.query(Scope) \
                .filter(Scope.read_scope.in_([scope["name"] for scope in user["session_scopes"]])).all()), Category.scopes == None)) \
...
def remove_category(category_id: int, user: dict[str, str] = Depends(UnionAuth(["services.category.delete"]))):
...
def update_category(category_inp: CategoryUpdate, category_id: int, user: dict[str, str] = Depends(UnionAuth(["services.category.update"]))):
    category = db.session.query(Category).filter(Category.id == category_id).one_or_none()
    last_category = db.session.query(Category).order_by(Category.order.desc()).first()
    if not category:
        raise HTTPException(status_code=404, detail="Category does not exist")
