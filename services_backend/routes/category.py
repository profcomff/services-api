...
def get_categories(
    ...
    return [
        ...
        for row in list(filter(None, [categ if (set(user_scopes).intersection([scope.__dict__["name"] for scope in categ.scopes]) or (categ.scopes == [])) else None for categ in db.session.query(Category).all()]))
    ]
